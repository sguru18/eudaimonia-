...
const KEYS = {
  ...
  EXPENSE_CATEGORIES: "@garden/expense_categories",
  SUBSCRIPTIONS: "@garden/subscriptions",
  ...
  RECURRING_TIME_BLOCKS: "@garden/recurring_time_blocks",
...
export const mealService = {
  ...
  async create(
    meal: Omit<Meal, "id" | "created_at" | "updated_at" | "user_id">
  ): Promise<Meal | null> {
    try {
...
export const groceryService = {
  ...
  async create(
    item: Omit<GroceryItem, "id" | "created_at" | "updated_at" | "user_id">
...
export const expenseCategoryService = {
  async getAll(): Promise<ExpenseCategory[]> {
    try {
      ...
      let unique = data
        ? Array.from(new Map(data.map((cat) => [cat.id, cat])).values())
        : [];

      // Additional deduplication by name (case-insensitive) - keep first occurrence by sort_order
      const nameMap = new Map<string, ExpenseCategory>();
      for (const cat of unique) {
        const key = `${cat.name.toLowerCase().trim()}_${userId}`;
        const existing = nameMap.get(key);
        if (!existing || cat.sort_order < existing.sort_order) {
          nameMap.set(key, cat);
        }
      }
      unique = Array.from(nameMap.values()).sort(
        (a, b) => a.sort_order - b.sort_order
      );

      if (unique.length > 0) {
        await setLocalData(KEYS.EXPENSE_CATEGORIES, unique);
      }

      return unique;
    } catch (error) {
      console.error("Error fetching expense categories:", error);
      const local = await getLocalData<ExpenseCategory>(
        KEYS.EXPENSE_CATEGORIES
      );
      // Also deduplicate local data by id and name
      const byId = Array.from(
        new Map(local.map((cat) => [cat.id, cat])).values()
      );
      const nameMap = new Map<string, ExpenseCategory>();
      for (const cat of byId) {
  ...
  async ensureDefaults(): Promise<ExpenseCategory[]> {
    try {
      const userId = await getCurrentUserId();

      // Check if user has any categories - fetch fresh from DB to avoid duplicates
      ...
      // Deduplicate by id first
      let unique = existing
        ? Array.from(new Map(existing.map((cat) => [cat.id, cat])).values())
        : [];

      // Additional deduplication: if same name exists multiple times, keep only the first one (by sort_order)
      const nameMap = new Map<string, ExpenseCategory>();
      for (const cat of unique) {
        const key = `${cat.name.toLowerCase().trim()}_${userId}`;
        const existing = nameMap.get(key);
      ...
      const defaults = DEFAULT_EXPENSE_CATEGORIES.map((cat, index) => ({
        user_id: userId,
        name: cat.name,
        color: cat.color,
        is_default: true,
      ...
      if (data) {
        await setLocalData(KEYS.EXPENSE_CATEGORIES, data);
      }
  ...
  async create(
    ...
    try {
      ...
      if (data) {
        const categories = await getLocalData<ExpenseCategory>(
          KEYS.EXPENSE_CATEGORIES
        );
        await setLocalData(KEYS.EXPENSE_CATEGORIES, [...categories, data]);
      }
  ...
  async update(
    ...
    try {
      ...
      if (data) {
        ...
        await setLocalData(KEYS.EXPENSE_CATEGORIES, updated);
      }
...
export const expenseService = {
  ...
  async getByDateRangeWithCategories(
    startDate: string,
    endDate: string
    ...
    try {
      ...
      console.error("Error fetching expenses by date with categories:", error);
      return [];
    }
  },
  ...
  async getByDate(date: string): Promise<Expense[]> {
    try {
      ...
      console.error("Error fetching expenses by date:", error);
      const allExpenses = await getLocalData<Expense>(KEYS.EXPENSES);
      return allExpenses.filter((e) => e.date === date);
    }
  },

  async create(
    expense: Omit<Expense, "id" | "created_at" | "updated_at" | "user_id">
  ...
  async getTotalsByCategory(
    ...
    for (const expense of expenses) {
      ...
      totals.set(expense.category_id, current + expense.amount);
    }
...
export const subscriptionService = {
  async getAll(): Promise<Subscription[]> {
    try {
      ...
      if (data) {
        await setLocalData(KEYS.SUBSCRIPTIONS, data);
      }

      return data || [];
    } catch (error) {
      console.error("Error fetching subscriptions:", error);
      return getLocalData<Subscription>(KEYS.SUBSCRIPTIONS);
    }
  },

  async getAllWithCategories(): Promise<SubscriptionWithCategory[]> {
    try {
      const { data, error } = await supabase
        .from("subscriptions")
        .select("*, category:expense_categories(*)")
      ...
      console.error("Error fetching subscriptions with categories:", error);
      return [];
    }
  },
  ...
  async getActive(): Promise<Subscription[]> {
    try {
      ...
      const all = await getLocalData<Subscription>(KEYS.SUBSCRIPTIONS);
  ...
  async create(
    ...
    try {
      ...
      if (data) {
        const subscriptions = await getLocalData<Subscription>(
          KEYS.SUBSCRIPTIONS
        );
        await setLocalData(KEYS.SUBSCRIPTIONS, [...subscriptions, data]);
      }
  ...
  async update(
    ...
    try {
      ...
      if (data) {
        ...
        const updated = subscriptions.map((s) => (s.id === id ? data : s));
        await setLocalData(KEYS.SUBSCRIPTIONS, updated);
      }
  ...
  async toggleActive(id: string): Promise<Subscription | null> {
    try {
      const subscriptions = await this.getAll();
      const subscription = subscriptions.find((s) => s.id === id);
      if (!subscription) return null;

      return await this.update(id, { is_active: !subscription.is_active });
    } catch (error) {
  ...
  async getMonthlyTotal(): Promise<number> {
    ...
    return active.reduce((sum, s) => sum + s.amount, 0);
  },
};
...
export const habitService = {
  ...
  async create(
    habit: Omit<Habit, "id" | "created_at" | "updated_at" | "user_id">
  ): Promise<Habit | null> {
    try {
      const userId = await getCurrentUserId();
      const { data, error } = await supabase
  ...
  async ensureWeekHabits(
    ...
    try {
      ...
      const copiedCount = await this.copyToWeek(
        previousWeekStart,
        weekStartDate
      );
...
export const habitCompletionService = {
  async getByHabit(habitId: string): Promise<HabitCompletion[]> {
    try {
      ...
      const allCompletions = await getLocalData<HabitCompletion>(
        KEYS.HABIT_COMPLETIONS
      );
  ...
  async toggle(
    ...
    try {
      ...
      if (existing) {
        ...
        const completions = await getLocalData<HabitCompletion>(
          KEYS.HABIT_COMPLETIONS
        );
        await setLocalData(
          KEYS.HABIT_COMPLETIONS,
          completions.filter((c) => c.id !== existing.id)
        );
...
export const habitReminderService = {
  async getByWeek(weekStartDate: string): Promise<HabitReminder | null> {
    try {
      ...
      const allReminders = await getLocalData<HabitReminder>(
        KEYS.HABIT_REMINDERS
      );
      return (
        allReminders.find((r) => r.week_start_date === weekStartDate) || null
      );
    }
  },
  ...
  async upsert(
    ...
    try {
      ...
      if (existing) {
        ...
        if (data) {
          const reminders = await getLocalData<HabitReminder>(
            KEYS.HABIT_REMINDERS
          );
          await setLocalData(
            KEYS.HABIT_REMINDERS,
...
export const reflectionService = {
  ...
  async create(
    reflection: Omit<Reflection, "id" | "created_at" | "updated_at" | "user_id">
...
export const notificationSettingsService = {
  ...
  async getById(id: string): Promise<NotificationSetting | null> {
    try {
      ...
      const allSettings = await getLocalData<NotificationSetting>(
  ...
  async create(
    ...
    try {
      ...
      if (data) {
        const settings = await getLocalData<NotificationSetting>(
          KEYS.NOTIFICATION_SETTINGS
        );
...
export const StretchingRoutinesDB = {
  async getAll(): Promise<StretchingRoutine[]> {
    try {
      // Try local first
      const localRoutines = await getLocalData<StretchingRoutine>(
  ...
  async getById(id: string): Promise<StretchingRoutine | null> {
    try {
      ...
      const routines = await getLocalData<StretchingRoutine>(
        KEYS.STRETCHING_ROUTINES
      );
  ...
  async create(
    routine: Omit<
      ...
      "id" | "created_at" | "updated_at" | "user_id"
    >
  ): Promise<StretchingRoutine | null> {
    try {
      const userId = await getCurrentUserId();
      const { data, error } = await supabase
        .from("stretching_routines")
  ...
  async update(
    id: string,
    updates: Partial<StretchingRoutine>
  ): Promise<StretchingRoutine | null> {
    try {
      const { data, error } = await supabase
        .from("stretching_routines")
...
export const StretchingExercisesDB = {
  async getByRoutineId(routineId: string): Promise<StretchingExercise[]> {
    try {
      // Try local first
      const localExercises = await getLocalData<StretchingExercise>(
        KEYS.STRETCHING_EXERCISES
      );
      const filteredLocal = localExercises.filter(
      ...
      if (data && data.length > 0) {
        ...
        const otherExercises = localExercises.filter(
          (e) => e.routine_id !== routineId
        );
        await setLocalData(KEYS.STRETCHING_EXERCISES, [
          ...otherExercises,
          ...data,
        ]);
        return data;
      ...
      const exercises = await getLocalData<StretchingExercise>(
        KEYS.STRETCHING_EXERCISES
      );
  ...
  async create(
    exercise: Partial<StretchingExercise>
  ): Promise<StretchingExercise | null> {
    try {
      const { data, error } = await supabase
        .from("stretching_exercises")
        .insert(exercise)
  ...
  async update(
    id: string,
    updates: Partial<StretchingExercise>
  ): Promise<StretchingExercise | null> {
    try {
      const { data, error } = await supabase
        .from("stretching_exercises")
  ...
  async deleteByRoutineId(routineId: string): Promise<boolean> {
    try {
      ...
      await setLocalData(
        KEYS.STRETCHING_EXERCISES,
        exercises.filter((e) => e.routine_id !== routineId)
      );
...
export const priorityService = {
  ...
  async create(
    priority: Omit<Priority, "id" | "created_at" | "updated_at" | "user_id">
  ...
  async delete(id: string): Promise<boolean> {
    try {
      ...
      await supabase.from("priority_weeks").delete().eq("priority_id", id);
      ...
      await setLocalData(
        KEYS.PRIORITIES,
        priorities.filter((p) => p.id !== id)
      );

      const priorityWeeks = await getLocalData<PriorityWeek>(
        KEYS.PRIORITY_WEEKS
      );
      await setLocalData(
        KEYS.PRIORITY_WEEKS,
        priorityWeeks.filter((pw) => pw.priority_id !== id)
      );
...
export const priorityWeekService = {
  async getByWeek(weekStartDate: string): Promise<PriorityWithRank[]> {
    try {
      ...
      // Combine them
      const result: PriorityWithRank[] = priorityWeeksData
        .map((pw) => {
      ...
      // Fallback to local
      const allPriorityWeeks = await getLocalData<PriorityWeek>(
        KEYS.PRIORITY_WEEKS
      );
      const allPriorities = await getLocalData<Priority>(KEYS.PRIORITIES);
    ...
    try {
      ...
      if (existing) {
        ...
        if (data) {
          ...
          await setLocalData(
            KEYS.PRIORITY_WEEKS,
            priorityWeeks.map((pw) => (pw.id === data.id ? data : pw))
          );
        }
  ...
  async updateWeekRanks(
    weekStartDate: string,
    priorityRanks: { priorityId: string; rankOrder: number }[]
  ): Promise<boolean> {
    try {
  ...
  async removeFromWeek(
    ...
    try {
      ...
      await setLocalData(
        KEYS.PRIORITY_WEEKS,
        priorityWeeks.filter(
          (pw) =>
            !(
              pw.priority_id === priorityId &&
              pw.week_start_date === weekStartDate
            )
        )
      );
  ...
  async getWeeksWithPriorities(
    startDate: string,
    endDate: string
  ): Promise<Map<string, PriorityWithRank[]>> {
    try {
      ...
      const priorityIds = [
        ...new Set(priorityWeeksData.map((pw) => pw.priority_id)),
      ];
      const { data: prioritiesData, error: pError } = await supabase
        .from("priorities")
        .select("*")
      ...
      for (const pw of priorityWeeksData) {
        ...
        const priorityWithRank: PriorityWithRank = {
          ...priority,
          rank_order: pw.rank_order,
        };
...
// Recurring Time Block operations
export const recurringTimeBlockService = {
  async getAll(): Promise<RecurringTimeBlock[]> {
    try {
      const { data, error } = await supabase
      ...
      if (data) {
        await setLocalData(KEYS.RECURRING_TIME_BLOCKS, data);
      }

      return data || [];
    } catch (error) {
      console.error("Error fetching recurring time blocks:", error);
      return getLocalData<RecurringTimeBlock>(KEYS.RECURRING_TIME_BLOCKS);
    }
  },

  async getActive(): Promise<RecurringTimeBlock[]> {
    try {
      const { data, error } = await supabase
        .from("recurring_time_blocks")
        .select("*")
      ...
      console.error("Error fetching active recurring time blocks:", error);
      const all = await getLocalData<RecurringTimeBlock>(
        KEYS.RECURRING_TIME_BLOCKS
      );
      return all.filter((b) => b.is_active);
    }
  },

  async getByDayOfWeek(dayIndex: number): Promise<RecurringTimeBlock[]> {
    // dayIndex: 0 = Sunday, 1 = Monday, etc.
    try {
      const allActive = await this.getActive();
      return allActive.filter((block) => block.days_of_week[dayIndex]);
    } catch (error) {
      console.error("Error fetching recurring blocks by day:", error);
      return [];
    }
  },

  async getById(id: string): Promise<RecurringTimeBlock | null> {
    try {
      const { data, error } = await supabase
        .from("recurring_time_blocks")
        .select("*")
      ...
      console.error("Error fetching recurring time block by id:", error);
      const allBlocks = await getLocalData<RecurringTimeBlock>(
        KEYS.RECURRING_TIME_BLOCKS
      );
      return allBlocks.find((b) => b.id === id) || null;
    }
  ...
  async create(
    block: Omit<
      ...
      "id" | "created_at" | "updated_at" | "user_id"
    >
  ): Promise<RecurringTimeBlock | null> {
    try {
      const userId = await getCurrentUserId();
      const { data, error } = await supabase
        .from("recurring_time_blocks")
      ...
      if (data) {
        const blocks = await getLocalData<RecurringTimeBlock>(
          KEYS.RECURRING_TIME_BLOCKS
        );
        await setLocalData(KEYS.RECURRING_TIME_BLOCKS, [...blocks, data]);
      }

      return data;
    } catch (error) {
      console.error("Error creating recurring time block:", error);
      return null;
    }
  },
  ...
  async update(
    id: string,
    updates: Partial<RecurringTimeBlock>
  ): Promise<RecurringTimeBlock | null> {
    try {
      const { data, error } = await supabase
        .from("recurring_time_blocks")
      ...
      if (data) {
        ...
        const updatedBlocks = blocks.map((b) => (b.id === id ? data : b));
        await setLocalData(KEYS.RECURRING_TIME_BLOCKS, updatedBlocks);
      }

      return data;
    } catch (error) {
      console.error("Error updating recurring time block:", error);
      return null;
    }
  },
  ...
  async delete(id: string): Promise<boolean> {
    try {
      ...
      console.error("Error deleting recurring time block:", error);
      return false;
    }
  },

  async toggleActive(id: string): Promise<RecurringTimeBlock | null> {
    try {
      const block = await this.getById(id);
      if (!block) return null;

      return await this.update(id, { is_active: !block.is_active });
    } catch (error) {
      console.error("Error toggling recurring time block:", error);
      return null;
    }
  },
};
